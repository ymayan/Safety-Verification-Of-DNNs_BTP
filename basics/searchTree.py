...
class searchTree:
    ...
    def parentIndex(self, xxx_todo_changeme): 
        (ci,pi) = xxx_todo_changeme
        for (k,d) in list(self.images.keys()): 
            if k == pi: 
                return (k,d)
    ...
    def addImages(self,model,ims):
        inds = [ i for (i,j) in list(self.images.keys()) if j == -1 ]
        index = max(inds) + 1
        for image in ims: 
            ...
            manipulated2 = set(self.manipulated[-1] + list(span.keys()))
    ...
    def removeProcessed(self,index):
        children = [ (k,p) for (k,p) in list(self.spans.keys()) if p == index[0] ]
        if children == []: 
