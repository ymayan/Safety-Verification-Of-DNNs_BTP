...
class dataCollection:
    ...
    def summarise(self):
        if len(self.l1Distance) == 0: 
            ...
            return 0
        art = sum(self.runningTime.values()) / len(list(self.runningTime.values())) 
        self.fileHandler.write("average running time: %s\n"%(art))
        amp = sum(self.manipulationPercentage.values()) / len(list(self.manipulationPercentage.values()))
        self.fileHandler.write("average manipulation percentage: %s\n"%(amp))
        eudist = sum(self.euclideanDistance.values()) / len(list(self.euclideanDistance.values()))
        self.fileHandler.write("average euclidean distance: %s\n"%(eudist))
        l1dist = sum(self.l1Distance.values()) / len(list(self.l1Distance.values()))
        self.fileHandler.write("average L1 distance: %s\n"%(l1dist))
        l0dist = sum(self.l0Distance.values()) / len(list(self.l0Distance.values()))
