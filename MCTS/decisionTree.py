...
class decisionTree:
    ...
    def getAction(self,dims,currentNode):
        ...
        if self.tree[currentNode] != []:
            for act in (list(zip (*self.tree[currentNode])))[0]: 
                existingDims += list(self.actions[act][0].keys())
        ...
        if intersectSet == []: 
            ...
            e = intersectSet[0]
        for k,ls in self.actions.items():
            if e in list(ls[1].keys()) : 
                return (k, [ e2 for e2 in dims if e2 not in list(ls[1].keys()) ])
