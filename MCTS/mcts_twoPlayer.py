...
class mcts_twoPlayer:
    ...
    def bestChild(self,index):
        ...
        return max(iter(allValues.items()), key=operator.itemgetter(1))[0]
    ...
    def treeTraversal(self,index):
        if self.fullyExpanded[index] == True: 
            ...
            if self.player_mode == "adversary" and self.keypoint[index] == 0 : 
                ...
                nextIndex = np.random.choice(list(allValues.keys()), 1, p = [ x/sum(allValues2.values()) for x in list(allValues2.values())])[0]
            else: 
                nextIndex = np.random.choice(list(allValues.keys()), 1, p = [ x/sum(allValues.values()) for x in list(allValues.values())])[0]

            if self.keypoint[index] in list(self.usedActionsID.keys()) and self.keypoint[index] != 0 : 
            ...
            for k in list(self.usedActionsID.keys()): 
    ...
    def initialiseExplorationNode(self,index,availableActions):
        ...
        if self.keypoint[index] != 0: 
            for (actionId, (span,numSpan,_)) in availableActions[self.keypoint[index]].items() : #initialisePixelSets(self.model,self.image,list(set(self.spans[index].keys() + self.usefulPixels))): 
                self.indexToNow += 1
            ...
            for kp in list(self.keypoints.keys()) : #initialisePixelSets(self.model,self.image,list(set(self.spans[index].keys() + self.usefulPixels))): 
                self.indexToNow += 1
    ...
    def analysePixels(self):
        ...
        values = list(self.usefulPixels.values())
        images = []
        for v in values: 
            pixels = [ dim for key, value in self.usefulPixels.items() for dim in list(self.actions[key][0].keys()) if value >= v ]
    ...
    def addUsefulPixels(self,dims):
        for dim in dims: 
            if dim in list(self.usefulPixels.keys()): 
                    ...
                    if set(span.keys()).issubset(set(spanPath.keys())): 
                        ...
                        if newClass != self.originalClass and newConfident > effectiveConfidenceWhenChanging:
                            for k in list(span.keys()): 
                                spanPath.pop(k)
                                numSpanPath.pop(k)
        if len(list(lastSpanPath.keys())) != len(list(spanPath.keys())): 
