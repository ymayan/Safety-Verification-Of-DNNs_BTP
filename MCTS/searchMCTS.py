...
class searchMCTS:
    ...
    def collectUselessPixels(self,index):
        ...
        for childIndex in self.children[index]: 
            if self.cost[childIndex] == 0:
                diffPixels = [ x for x in list(self.spans[childIndex].keys()) if x not in list(self.spans[index].keys()) ] 
        ...
        for i in range(uselessNum): 
            k = max(iter(occurrence.items()), key=operator.itemgetter(1))[0]
    ...
    def treeTraversal(self,index):
        if self.fullyExpanded[index] == True: 
            ...
            nextIndex = max(iter(allValues.items()), key=operator.itemgetter(1))[0]
    ...
    def initialiseExplorationNode(self,index,availableActions):
        ...
        for (actionId, (span,numSpan,_)) in availableActions.items() : 
            self.indexToNow += 1
    ...
    def sampling(self,index,availableActions):
        ...
        for i in range(MCTS_multi_samples): 
            ...
            (childTerminated, val) = self.sampleNext(self.spans[index],self.numSpans[index],0,list(availableActions2.keys()),[])
    ...
    def showBestCase(self):
        return "(%s,%s)"%(self.bestCase[0],list(self.bestCase[1].keys()))
