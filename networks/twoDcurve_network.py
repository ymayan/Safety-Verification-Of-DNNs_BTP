...
def getConfig(model):
    ...
    config = list(zip(list(range(len(config))),config))
    return config 
...
def constructInputOutputArray3(x):
    return [ (list(zip(x1,x2,x3)))[0] for (x1,x2,x3) in zip(x[0],x[1],x[2])]
    
def constructInputOutputArray2(x):
    return [ (list(zip(x1,x2)))[0] for (x1,x2) in zip(x[0],x[1]) ]
