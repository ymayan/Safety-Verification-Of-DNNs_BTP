...
def conv_region_solve(nfeatures,nfilters,filters,bias,activations0,activations1,cl2,gl2,inds,nn):
    ...
    span = {}
    numSpan = {}
    if len((list(cl2.keys()))[0]) == 2: 
        for (x,y) in list(cl2.keys()):
    ...
    if inverseFunction == "area" :
        inds = [(k,x-x1,y-y1) for k in range(nfilters) for (l,x,y) in list(span.keys()) for x1 in range(filterSize) for y1 in range(filterSize) if x-x1 >= 0 and y-y1 >= 0 and x-x1 < activations1.shape[1] and y-y1 < activations1.shape[2] ]
    ...
    nextNumSpan = {}
    for k in inds:
        nextSpan[k] = np.max(list(span.values()))
        nextNumSpan[k] = np.max(list(numSpan.values()))
            ...
            for l in range(nfeatures):
                for x1 in range(filterSize):
                    for y1 in range(filterSize): 
                        ...
                        #print span.keys()
                        if (l,x+x1,y+y1) in list(span.keys()): 
                            #print (l,x+x1,y+y1)
